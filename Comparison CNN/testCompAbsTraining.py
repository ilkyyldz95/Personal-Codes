...
pred = total_model.predict(x_test)
te_acc_abs = compute_abs_accuracy(pred, y_test)
print('* Absolute label accuracy after both trainings: %0.2f%%' % (100 * te_acc_abs))

# COMP ACCURACY AFTER COMP&ABS TRAINING
# pred = total_model.predict(te_pairs[:, 0] - te_pairs[:, 1])
# get the output of f
f_out = K.function([total_model.layers[0].input],[total_model.layers[no_of_f_layers-1].output])
pred = f_out([te_pairs[:, 0] - te_pairs[:, 1]])
te_acc_comp = compute_comp_accuracy(np.asarray(pred), te_y)
