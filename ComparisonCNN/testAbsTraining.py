...
''' TEST THE NETWORK TRAINED FOR ABSOLUTE LABELS ONLY'''
...
json_file.close()
abs_model = model_from_json(loaded_model_json)
abs_model.load_weights("abs_label_model.h5")
print("Model loaded")

# ABS ACCURACY AFTER COMP&ABS TRAINING
pred = abs_model.predict(te_x)
te_acc_abs = compute_abs_accuracy(pred, te_abs)
print('* Absolute label accuracy after absolute label training: %0.2f%%' % (100 * te_acc_abs))

# COMP ACCURACY AFTER COMP&ABS TRAINING
# get the output of f
f_out = K.function([abs_model.layers[0].input],[abs_model.layers[no_of_f_layers-1].output])
...
print('* Comparison label accuracy after absolute label training: %0.2f%%' % (100 * te_acc_comp))
