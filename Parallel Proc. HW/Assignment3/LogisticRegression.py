...
def writeBeta(output,beta):
    ...
    """
    beta_sort = sorted(beta.items(), key=operator.itemgetter(1))
    with open(output,'w') as fh:
        for keyTp in beta_sort:
            fh.write(str(keyTp))
...
def test(data,beta):
    ...
    """
    P_true = []  #true label of positive estimates
    N_true = []  #true label of negative estimates
    TP = 0.0
    FP = 0.0
    TN = 0.0
    FN = 0.0
    ...
    accuracy = (TP + TN) / (len(P_true) + len(N_true))
    precision = TP / (TP + FP)
    recall = TP / (TP + FN)
