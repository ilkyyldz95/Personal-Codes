...
def joinAndPredictAll(R,U,V,N):
    ...
    R_i_jr = R.map(lambda (i,j,rij): (i, (j, rij)))
    # join and make common keys j
    j_iur = R_i_jr.join(U, numPartitions=N).map(lambda (i,((j, rij), ui)): (j, (i, ui, rij)))
    # join all and apply pred_diff(rij, ui, vj)
    return j_iur.join(V, numPartitions=N).map(lambda (j, ((i, ui, rij), vj)): (i, j, pred_diff(rij, ui, vj), ui, vj))
...
def SE(joinedRDD):
    ...
    """
    return joinedRDD.map(lambda (i,j,delij,ui,vj): delij**2).reduce(lambda del1, del2: del1 + del2)
...
def normSqRDD(profileRDD,param):
    """ Receives as input an RDD of profiles (e.g., U) as well as a parameter (e.g., λ) and computes the square of norms:
        ...
        λ Σ_i ||ui||_2^2         
    """
    return param * profileRDD.map(lambda (i,ui): np.linalg.norm(ui)**2).reduce(lambda u1, u2: u1+u2)
...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    grad_u_SE = joinedRDD.map(lambda (i,j,delij,ui,vj): (i, gradient_u(delij, ui, vj))).\
                reduceByKey(lambda grad1, grad2:grad1+grad2, N)
    # join with previous ui
    i_ui_gradSE = joinedRDD.map(lambda (i, j, delij, ui, vj): (i, ui)).reduceByKey(lambda ui, temp: ui, N).join(grad_u_SE, N)
    # add regularization
    i_ui_gradRSE = i_ui_gradSE.mapValues(lambda (ui,grad_ui): (ui, grad_ui + 2*lam*ui))
    # update ui
    return i_ui_gradRSE.map(lambda (i, (ui, grad_ui)): (i, ui - gamma * grad_ui))
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    grad_v_SE = joinedRDD.map(lambda (i,j,delij,ui,vj): (j, gradient_v(delij, ui, vj))).\
                reduceByKey(lambda grad1, grad2:grad1+grad2, N)
    # join with previous vj
    j_vj_gradSE = joinedRDD.map(lambda (i, j, delij, ui, vj): (j, vj)).reduceByKey(lambda vj, temp: vj, N).join(grad_v_SE, N)
    # add regularization
    j_vj_gradRSE = j_vj_gradSE.mapValues(lambda (vj,grad_vj): (vj, grad_vj + 2*mu*vj))
    # update ui
    return j_vj_gradRSE.map(lambda (j, (vj, grad_vj)): (j, vj - gamma * grad_vj))
