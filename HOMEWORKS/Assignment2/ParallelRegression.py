...
def F(data,beta,lam = 0):
    ...
    """
    # map every data pair to f(x,y,beta)
    f_rdd = data.map(lambda (x,y): f(x,y,beta))
    # sum all function values
    f_sum = 1.0 * f_rdd.reduce(lambda x,y: x + y) / f_rdd.count()
    beta_sum = np.sum(np.power(beta, 2))
    return f_sum + lam * beta_sum
...
def gradient(data,beta,lam = 0):
    ...
    """
    # calculate the local gradient for every data pair
    grad_rdd = data.map(lambda (x, y): localGradient(x,y,beta))
    # sum all gradient values
    grad_sum = 1.0 * grad_rdd.reduce(lambda x, y: x + y) / grad_rdd.count()
    # gradient of the regularization term
    grad_lam = 2 * lam * beta
    return grad_sum + grad_lam
...
def test(data,beta):
    ...
    f_rdd = data.map(lambda (x, y): f(x, y, beta))
    # sum all function values
    return 1.0 * f_rdd.reduce(lambda x, y: x + y) / f_rdd.count()
...
def train(data,beta_0, lam,max_iter,eps):
    ...
    t_start = time()
    # find F(beta)
    fun = lambda beta_val: F(data, beta_val, lam)
    iteration = 0
    beta = beta_0
    grad = gradient(data,beta,lam)

    while iteration < max_iter and np.linalg.norm(grad) >= eps:
        # find gamma_k
        gamma = lineSearch(fun, beta, grad, a=0.2,b=0.6)
        # update beta
        beta = beta - gamma * grad
        # update gradient
        grad = gradient(data, beta, lam)
        iteration += 1
        print 'iter:' + str(iteration) + ',elapsed time:' + str(time()-t_start) + \
                ',F(beta):' + str(fun(beta)) + ',norm(delF(beta)):' + str(np.linalg.norm(grad))


    grad_norm = np.linalg.norm(grad)
    return beta, grad_norm, iteration
